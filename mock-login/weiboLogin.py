...
from config import USER_NAME_WB, PASSWORD_WB
...
class WeiBo(object):
    ...
    def prelogin_params(self):
        params = {
            ...
            "su": self.user_encrypt(USER_NAME_WB),
            "rsakt": "mod",
            "checkpin": "1",
    ...
    def get_qrcode(self, pcid):
        ...
        with open("qrcode.png", "wb") as fp:
            fp.write(resq.content)

        qrcode = input("please input qrcode:")
        return qrcode

    def submit_login(self, servertime, nonce, pubKey, rsakv, pcid):
        qrcode = self.get_qrcode(pcid)
        # print(qrcode)
        sp = self.pass_encrypt(PASSWORD_WB, servertime, nonce, pubKey)
        data = {
            "entry": "weibo",
            "gateway": "1",
            "from": "",
        ...
        if response.text.find("杯子是方的") != -1:
            print(10005, "Login Succcess")
            print("登录成功")
        else:
...
if __name__ == "__main__":
    weibo = WeiBo()
    pcid, servertime, nonce, pubKey, rsakv = weibo.prelogin_params()
